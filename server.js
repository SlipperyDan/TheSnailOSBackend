...
async function fetchRiotApi(url, apiKey, timeoutMs = 1e4) {
...
app.get("/api/riot/player/:region/:gameName/:tagLine/matches", async (req, res) => {
  ...
  try {
    ...
    const matchPromises = matchIds.map(
      (id) => fetchRiotApi(`https://${region}.api.riotgames.com/lol/match/v5/matches/${id}`, apiKey).catch(() => null)
    );
    const matchesData = await Promise.all(matchPromises);
    const history = matchesData.filter((m) => m && m.info).map((m) => {
      const info = m.info;
